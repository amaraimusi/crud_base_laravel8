...
class MsgBoardEvalType extends AppModel
{
	protected $table = 'msg_board_eval_types'; // 紐づけるテーブル名
	...
	public function getData($crudBaseData){
	    ...
	    $sql =
	    "
				SELECT SQL_CALC_FOUND_ROWS MsgBoardEvalType.* {$outer_selects_str}
				FROM {$main_tbl_name} AS MsgBoardEvalType
				{$outer_join_str}
				WHERE {$conditions}
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_main'])){
			$cnds[]="CONCAT( IFNULL(MsgBoardEvalType.msg_board_eval_type_name, '') ,IFNULL(MsgBoardEvalType.note, '')) LIKE '%{$kjs['kj_main']}%'";
		}
		
		// CBBXS-1003
		if(!empty($kjs['kj_id']) || $kjs['kj_id'] ==='0' || $kjs['kj_id'] ===0){
			$cnds[]="MsgBoardEvalType.id = {$kjs['kj_id']}";
		}
		if(!empty($kjs['kj_eval_type_code'])){
			$cnds[]="MsgBoardEvalType.eval_type_code LIKE '%{$kjs['kj_eval_type_code']}%'";
		}
		if(!empty($kjs['kj_eval_value1'])){
			$cnds[]="MsgBoardEvalType.eval_value >= {$kjs['kj_eval_value1']}";
		}
