...
jQuery(()=> {
	...
	$('#msg_board_user_eval_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	// CBBXS-2023
	// 評価種別IDリストJSON
	let evalTypeIdList = crudBaseData.masters.evalTypeIdList;
	disFilData['eval_type_id'] ={'fil_type':'select','option':{'list':evalTypeIdList}};
	...
	crudBase.crudBaseBulkAdd.init(
		[
			...
			{'field':'eval_type_id', 'inp_type':'select', 'list':evalTypeIdList, 'def':0}, 
			{'field':'sort_no', 'inp_type':'textarea'}, 
			{'field':'delete_flg', 'inp_type':'textarea'}, 

			// CBBXE
			
//			{'field':'msg_board_user_eval_group', 'inp_type':'select', 'list':msg_board_user_evalGroupList, 'def':2}, 
//			{'field':'msg_board_user_eval_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'msg_board_user_eval/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーしたメッセージボード・ユーザー評価名、メッセージボード・ユーザー評価数値を貼り付けてください。（タブ区切りテキスト）\n(例)\nメッセージボード・ユーザー評価名A\t100\nメッセージボード・ユーザー評価名B\t101\n",
		}
	);
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('msg_board_user_eval_date');
}
