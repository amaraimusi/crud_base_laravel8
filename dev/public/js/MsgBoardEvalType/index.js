...
jQuery(()=> {
	...
	$('#msg_board_eval_type_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	// CBBXS-2023
	// 反対評価種別IDリストJSON
	let converselyEvalTypeIdList = crudBaseData.masters.converselyEvalTypeIdList;
	disFilData['conversely_eval_type_id'] ={'fil_type':'select','option':{'list':converselyEvalTypeIdList}};
	...
	crudBase.crudBaseBulkAdd.init(
		[
			...
			{'field':'conversely_eval_type_id', 'inp_type':'select', 'list':converselyEvalTypeIdList, 'def':0}, 
...
//			{'field':'msg_board_eval_type_group', 'inp_type':'select', 'list':msg_board_eval_typeGroupList, 'def':2}, 
//			{'field':'msg_board_eval_type_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'msg_board_eval_type/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーしたメッセージボード・評価種別名、メッセージボード・評価種別数値を貼り付けてください。（タブ区切りテキスト）\n(例)\nメッセージボード・評価種別名A\t100\nメッセージボード・評価種別名B\t101\n",
		}
	);
...
function _getValidMethods(){
	let methods = {
			...
			kj_eval_type_code:(cbv, value)=>{
				let err = '';
				// 文字数バリデーション
				if(!cbv.isMaxLength(value, 12)){
					err = '12文字以内で入力してくだい。';
				}
				return err;
			},
			kj_icon_fn:(cbv, value)=>{
				let err = '';
				// 文字数バリデーション
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('msg_board_eval_type_date');
}
