...
<head>
	...
	<title>メッセージボード・評価種別管理画面</title>
	
	<script src="{{ asset('/js/app.js') }}" defer></script>
	<script src="{{ $crud_base_js . $ver_str }}" defer></script>
	<script src="{{ asset('/js/MsgBoardEvalType/index.js')  . $ver_str }}" defer></script>
	...
	<link href="{{ asset('/css/MsgBoardEvalType/index.css') . $ver_str }}" rel="stylesheet">
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<li class="breadcrumb-item active" aria-current="page">メッセージボード・評価種別管理画面</li>
  </ol>
</nav>
...
<div class="cb_func_line">
	...
	<!-- 検索条件入力フォーム -->
	<div class="form_kjs" id="msg_board_eval_typeIndexForm" method="post" accept-charset="utf-8">
		
		<?php $cbh->inputKjMain('kj_main','',null,'メッセージボード・評価種別名、備考を検索する');?>
	...
	<div id="main_tools" style="margin-bottom:10px;margin-top:4px">
		<div style="display:inline-block;width:75%; ">
			<?php 
				...
				// CSVエクスポート機能
	 			$csv_dl_url =  'msg_board_eval_type/csv_download';
...
<!-- 一覧テーブル -->
<table id="msg_board_eval_type_tbl" class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >評価タイプコード: </div>
			<div class='cbf_input'>
				<input type="text" name="eval_type_code" class="valid " value=""  maxlength="12" title="12文字以内で入力してください" />
				<label class="text-danger" for="eval_type_code"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >評価値: </div>
			<div class='cbf_input'>
				<input type="text" name="eval_value" class="valid" value="" pattern="^[+-]?([0-9]*[.])?[0-9]+$" maxlength="11" title="数値を入力してください" />
		...
		<div class="cbf_inp_wrap" style="float:left">
			...
			<div class='cbf_input' style="width:180px;height:auto;">
				<label for="icon_fn_n" class="fuk_label" >
					<input type="file" id="icon_fn_n" class="icon_fn" style="display:none" accept="image/*" title="画像ファイルをドラッグ＆ドロップ(複数可)"  data-inp-ex='image_fuk' data-fp='' />
	...
	<tr id="ajax_crud_edit_form" class="crud_base_form" style="display:none"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >評価値: </div>
			<div class='cbf_input'>
				<input type="text" name="eval_value" class="valid " value=""  maxlength="11" title="11文字以内で入力してください" />
				<label class="text-danger" for="eval_value"></label>
			</div>
		</div>
		...
		<div class="cbf_inp_wrap" style="float:left">
			<div class='cbf_inp_label_long' >アイコンファイル名: </div>
			<div class='cbf_input' style="width:180px;height:auto;">
				<label for="icon_fn_e" class="fuk_label" >
					<input type="file" id="icon_fn_e" class="icon_fn" style="display:none" accept="image/*" title="画像ファイルをドラッグ＆ドロップ(複数可)"  data-inp-ex='image_fuk' data-fp='' />
					<span class='fuk_msg' style="padding:20%">画像ファイルをドラッグ＆ドロップ(複数可)</span>
				</label>
			</div>
		</div>
...
<div id="ajax_crud_delete_form" class="panel panel-danger" style="display:none">
	...
	<table><tbody>
		...
		<tr><td>メッセージボード・評価種別名: </td><td>
			<span class="msg_board_eval_type_name"></span>
		</td></tr>
